# Copied from locost-summarization/models.py
...
class LOCOSTLayerSelfAttention_Mamba(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Any,  # to accept past_key_value and use_cache kwargs
        ...
        # Mamba forward (does not take attention_mask or output_attentions)
        ssm_hidden_states = self.ssm_layer(normed_hidden_states)
...
class LOCOSTBlock_Mamba(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False):
        ...
        if self.is_decoder:
            ...
            self_attn = LOCOSTLayerSelfAttention_Mamba(config)
    ...
    def forward(
        ...
        # Mamba case: returns (hidden_states, None, None, None)
        hidden_states = self_attn_output[0]
        present_key_value_state = self_attn_output[1]
...
class LOCOSTPreTrainedModel_Mamba(PreTrainedModel):
    config_class = LOCOSTConfig_Mamba
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, LongT5LayerNorm):
        ...
        elif isinstance(module, (LOCOSTModel_Mamba, LOCOSTForConditionalGeneration_Mamba)):
        ...
        elif isinstance(module, LongT5TransientGlobalAttention):
            ...
            # Initialize Mamba params
            for name, param in module.named_parameters():
                if param.dim() > 1:
                    # Linear/conv weights
                    nn.init.normal_(param, mean=0.0, std=factor * (self.config.d_model ** -0.5))
                else:
                    # Biases or 1D params
                    nn.init.zeros_(param)

class LOCOSTStack_Mamba(LOCOSTPreTrainedModel_Mamba):
    def __init__(self, config, embed_tokens=None):
        ...
        # local_radius is S4-specific, but we leave it here
        # in case other components still use it.
        self.local_radius = getattr(config, "local_radius", None)
        self.block_len = self.local_radius + 1 if self.local_radius is not None else None

        # Use Mamba-enabled block instead of S4/T5
        self.block = nn.ModuleList(
            [
                LOCOSTBlock_Mamba(config, has_relative_attention_bias=(i == 0))
    ...
    def forward(
        ...
        # For caching in decoders (not really used in Mamba encoder-only mode)
        mask_seq_length = (
        ...
        # Attention mask logic (kept, but Mamba ignores it internally)
        if self.is_decoder:
            extended_attention_mask = self.get_extended_attention_mask(
                attention_mask, input_shape, inputs_embeds.device
            )
        elif getattr(self.config, "encoder_attention_type", None) == "local":
        ...
        for i, (layer_module, past_key_value) in enumerate(
            ...
            # Mamba outputs don’t produce biases/attn weights, so these stay None
...
class LOCOSTModel_Mamba(LOCOSTPreTrainedModel_Mamba):
    ...
    def __init__(self, config: LOCOSTConfig_Mamba):
        super().__init__(config)
        ...
        self.encoder = LOCOSTStack_Mamba(encoder_config, self.shared)
        ...
        self.decoder = LOCOSTStack_Mamba(decoder_config, self.shared)
    ...
    def _prune_heads(self, heads_to_prune):
        """
        Heads pruning is not meaningful in Mamba, since it has no attention heads.
    ...
    def forward(
        ...
        r"""
        Forward pass for the LOCOST model using Mamba layers in both encoder and decoder.
        """
        use_cache = use_cache if use_cache is not None else self.config.use_cache
        return_dict = (
            return_dict if return_dict is not None else self.config.use_return_dict
...
class LOCOSTForConditionalGeneration_Mamba(LOCOSTPreTrainedModel_Mamba):
    _keys_to_ignore_on_load_missing = [
        r"encoder.embed_tokens.weight",
        r"decoder.embed_tokens.weight",
        r"lm_head.weight",
    ...
    def forward(
        ...
        # Shift labels if no decoder inputs provided
        if (
            labels is not None
    ...
    def _reorder_cache(self, past_key_values, beam_idx):
        # For Mamba, caching is a no-op but we keep this for API compatibility
        if past_key_values is None:
